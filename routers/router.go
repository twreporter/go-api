...
func SetupRouter(cf *controllers.ControllerFactory) (engine *gin.Engine) {
	...
	v1Group.POST(fmt.Sprintf("/%s", globals.SendActivationAuthRoutePath), mailMiddleware.ValidateAuthorization(), middlewares.SetCacheControl("no-store"), ginResponseWrapper(mailContrl.SendActivationAuth))
	...
	v2AuthGroup.GET("/activateauth", middlewares.SetCacheControl("no-store"), mc.ActivateAuthV2)
