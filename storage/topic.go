...
func (m *MongoStorage) GetTopics(qs interface{}, limit int, offset int, sort string, embedded []string) ([]models.Topic, error) {
	var topics []models.Topic
	if embedded == nil {
		embedded = []string{"relateds_meta", "leading_image", "leading_image_portrait", "leading_video", "og_image"}
	}

	err := m.GetDocuments(qs, limit, offset, sort, "topics", &topics)
