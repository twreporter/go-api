...
func GenerateActivateMailBody(mailAddress, activeToken, destination string) string {
	href := fmt.Sprintf("%s://%s:%s/activate?email=%s&token=%s&destination=%s", viper.GetString("consumersettings.protocol"), viper.GetString("consumersettings.host"), viper.GetString("consumersettings.port"), mailAddress, activeToken, destination)
...
func GenerateEncryptedPassword(password []byte) (string, error) {
	salt := []byte(viper.GetString("encryptsettings.salt"))
